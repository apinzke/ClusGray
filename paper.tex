...
\C{We compare the calculated fluxes to 95\% c.l. gamma-ray upper
  limits set by Fermi-LAT after 18 months of observations
...
\caption{\it Comparing the gamma-ray emission from different
  ...
  Virgo and Perseus, respectively. \C{In the lower panel of each
    figure we show differential spectral index where $\dd F_\gamma/\dd
    E_\gamma \sim E_\gamma^{-\alpha_\gamma}$.} The flux from the
  clusters is integrated out to $\rvir$ using a point spread function
  of $0.1\deg$ and includes the boost from substructures. We find that
  the lower GeV energy regime is most constraining due to the peak
  sensitivity of Fermi at these energies and use this regime to get
  upper limits on boost factors.}
 \label{fig:clu_comp}
\end{minipage}
\end{figure*}
...
\C{A quantity that is of great scientific interest is the CR pressure
  ...
  \cite{2007A&A...473...41E,2010MNRAS.409..449P}.  First we calculate
  the CR distribution function $f = C\,p^{-\alpha}$, where $C$ is the
  normalization of each power-law, $p=P/m_\rmn{p} c$ is the normalized
  CR proton momentum, and $\alpha$ is the spectral index. The CR
  pressure $P_\CR \propto C=\tilde{C}/\rho_\rmn{gas}$, where
  $\tilde{C}\propto M^{0.44}$ for clusters with a mass $\mvir\gtrsim
  10^{14}\,\msun$ \cite{2010MNRAS.409..449P}. We determine the
  mass scaling of the gas density through its relation to the enclosed
  gas fraction $f_\rmn{gas}$, and find that $\rho_\rmn{gas}\propto
  f_\rmn{gas} \propto M^{0.135}$ \cite{2009ApJ...693.1142S}. The
  thermal pressure is derived from the temperature in virial
  equilibrium, and scales as $P_\rmn{th}\propto k_\B T \propto G M/R
  \propto M^{0.67}$. Finally we can derive the mass scaling of the
  volume weighted relative CR pressure from
\begin{equation}
 <X_\CR> = \frac{<P_\CR>}{<P_\rmn{th}>} \propto \frac{<\rho_\rmn{gas}\,
   \tilde{C}>}{<k_\B T>} \propto M^{-0.06}\,,
\end{equation}
which is roughly a constant, i.e. almost independent on mass. Hence we
adopt a constant $<X_\CR>\approx 0.02$ derived from a representative
simulated cluster \cite{2008MNRAS.385.1211P}. Since the CRs that build
up the bulk of the CR pressure also give rise to gamma-ray emission
through the production of decaying pions, we use gamma-ray upper
limits from Fermi-LAT to constrain the volume weighted $X_\CR$. In
Fig.~\ref{fig:XCR} we show the volume weighted relative CR pressure
for 32 clusters, for which LAT upper limits that assume King profiles
are derived. For most of the clusters in the sample we can constrain
$X_\CR < (0.1-0.2)$ using the extended gamma-ray upper limits, where
the most constraining clusters are Norma and Coma with $X_\CR <
0.07$. Most of the 32 clusters have $X_\CR$ upper limits within a
factor few from the best constraining clusters, which suggests that
stacking of clusters will be able to improve these limits. This might
even be improved further using a scaled stacking with our model as a
prior. In previous work \cite{2010ApJ...717L..71A} where the 1.5 year
Fermi data was used to derive limits on $X_\CR$ they found on average
more constraining limits on the most massive clusters, while we find
better limits on the low mass clusters. This difference is explained
by the different modelling of $P_\CR$; they assumed a mass
independent CR pressure while we assume that $P_\CR\sim
M^{0.44}$. Furthermore, the gamma-ray flux predictions that we use in
this work to constrain $X_\CR$ assumes the true gas density
distribution for each cluster \footnote{Derived from the parameters in
  the extended HIFLUGCS catalogue.} that accounts for
cluster-to-cluster scatter, in contrast to the mass-to-luminosity
scaling relations that was used in previous works.}
...
clusters/groups but Fornax. \C{We conclude that due to the large
  ...
  Fermi to rule out the LP model nor to constrain the BM DM models
