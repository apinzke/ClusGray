...
\begin{abstract}
  ...
  clusters, 2) different representative benchmark models of supersymmetric DM, and 3) leptophilic
  models of dark matter (DM) annihilation that recently have been proposed to explain the positron
  ...
  the hadronization the annihilating neutralinos, internal bremsstrahlung, and inverse Compton
  emission from the cosmic microwave background as well as a realistic spatial and spectral
  ...
  for Fornax. Because for a given minimum substructure mass as given by the DM free-streaming scale,
  galaxy clusters are the largest collapsed objects which maximizes the boost of the smoothly
  distributed annihilation flux by a factor of more than 1000. Since the annihilation flux of
  substructures is mostly contributed by the regions around the virial radius, the resulting surface
  brightness profiles are almost flat. This makes it very challenging to detect this flux with
  imaging air Cherenkov telescopes since their sensitivity drops linearly with radius and they
  typically encounter have $5-10$ linear resolution elements across a cluster. Assuming cold dark
  matter with a substructure mass distribution down to an Earth mass, we rule out the leptophilic
  models in present form in 14 clusters, and limit the boost from Sommerfeld enhancement in Fornax
  to be less than 2 which corresponds to a value in the Milky Way of less than 5. Alternatively, if
  the Sommerfeld enhancement factor is realized in Nature, this would imply limiting substructure
  mass in Fornax of $\mlim > 10^2\,\msun$ corresponding to a Milky Way value of $\mlim > 5\times
  10^3\,\msun$ which would be evidence for an unknown form of warm dark matter. We find that Fermi
  three-year data will be able to constrain the smallest mass of substructures using the
  supersymmetric benchmark models or alternatively their cross sections for a given substructure
  boost. Although the Perseus and Ophiuchus clusters are expected to have the largest CR induced
  flux according to our semi-analytic model that is derived from cosmological hydrodynamical cluster
  simulations, their central active galactic nucleus as well as the Galactic foreground makes it
  hard to detect this flux in the Fermi band. The next to best targets to detect the CR flux are the
  Virgo, Coma, and Norma clusters -- the Fermi upper limits are closing in on our predictions and
  will soon start to constraining the underlying CR physics such as shock acceleration efficiencies
