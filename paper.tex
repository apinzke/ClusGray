...
... to be written...  if $\mlim \approx 10^{-6}\,\msun$ then the boost
  ...
  alternatively if SFE corrext then $\mlim > 10^2\,\msun$.
\end{abstract}
...
protons. In section two we discuss the theory of DM and gamma-rays. In
...
gamma-ray emission from various radiative processes. In section three
...
section four. To extend the analysis to more clusters and find
promesing targets we estimate the fluxes from DM and CRs of all
clusters in the extended HIFLUGCS sample in section five. In section
six we conclude and discuss our results.

% --- section:  --- %
\section{Theory}
\label{sect:theory}
We start the section by discussing two different, but well motivated DM
models; leptophilic and supersymmetric DM. In addition we present the
gamma-ray framework that is used to calculate the emission for these DM
models using an Einasto DM density profile and the expected
enhancement from DM substructures. In the end of this sections we
outline the framework of inverse Compton emission on both CMB photons
as well as photons from stars and dust.
...
1+\rho_{\sub,i}^2(r)/\rho^2(r)$ (see Sect.~\ref{sect:subst}). The DM
...
that $\rho_{-2} = \rhos/4$. [MIGHT REMOVE THIS, COULD NOT REPRODUCE IT, CAN
MOTIVATE THE FACTOR 4 FROM MATCHING THE MASSES FROM THE EINSASTO WITH
THE NFW INSTEAD] SHOW INTEGRATION, COMPUTE RMAX, DISCUSS
FIG 1+2 SMOOTH COMPONENT
...
and tidal effects depleating their central number densities. Since
the rate of which DM is annihilating depends on the density squared,
the resulting flux from substructures is boosted compared to the
smooth density distribution. Recent high resolution simulations
suggest a flux enhancement from substructures of the order of ten up
to a few 100 \cite{2008MNRAS.391.1685S, 2008JPhCS.125a2008K} for a MW
size halo. We use a double power-law function to fit \footnote{Our
  approach of fitting the scaling behavior of $\rmn{L_{sub}}(<r)$
  directly from numerical simulations self-consistently accounts for
  the radial dependence of the substructure concentration
  \cite{2008MNRAS.391.1685S}.} the luminosity from the smooth
component of substructures (i.e. substructures within substructures
are not included) inside radius $r$ which is determined for the
...
\begin{eqnarray}
  {\rm L_{sub}}(<x) &=& a_0\,C(\mvir)\,{\rm L_{200sm}}(\mvir)\,x^{a_1
    \displaystyle{x}^{a_2}}\,,\quad\rmn{where} \nonumber\\
   a_0 &=& 0.76\,,\quad a_1 = 0.95\,,\quad  a_2 = -0.27\,, \quad 
\rmn{and} \nonumber\\
\label{eq:LSub} \\
  C(\mvir) &=& 0.023\,\left(\frac{\mvir}{M_{\rm lim}}\right)^{0.226}\,.
\label{eq:Csub}
\end{eqnarray}
Here $a_i$ denote our fitted variables, ${\rm L_{200sm}}$ is the
luminosity from the smooth halo without substructures within $\rvir$
and $x= \left(\frac{r}{\rvir}\right)$. The theoretical smallest mass
of subhalos $M_{\rm lim}$ is determined from free streaming length of
DM (CHECK), where we assume that the mass power-law scaling relation
in Eq.~(\ref{eq:Csub}) is valid down to the free streaming mass of DM
halos. This is conventionally in the cold dark matter universe
...
behavior in the power spectrum on these scales(CHECK). We derive the
normalization function $C(\mvir)$ in Eq.~(\ref{eq:Csub}) from the
simulations in \cite{2008Natur.456...73S}:
\begin{equation}
\left(\frac{\mvir\,M_{\rm res,sim}}
{M_{\rm 200sim}\,M_{\rm lim}}\right)^{0.226}\,.
 \label{eq:C2}
\end{equation}
Here $M_{\rm 200sim}=1.9\times10^{12}\msun$ is the mass of the MW halo
in the simulation and $M_{\rm res,sim}=10^5\msun$ is the mass of the
smallest resolved subhalos in the MW simulation. For DM halos more
...
Finally we derive the squared density profile for the substructures
using
\begin{eqnarray}
\rho_\rmn{sub}^2(r) &=& \frac{\dd \rmn{L_{sub}}}{\dd V} \frac{1}{\rmn{L_{200sm}}}
\int_{\rvir} \dd r' 4\pi r'^2 \rho^2(r')\qquad\rmn{where}\nonumber\\
 \frac{\dd \rmn{L_{sub}}}{\dd V} &=& a_0\,a_1\,C(\mvir)\,x^{a_2+a_1\,x^{a_2}}\,
\left[\frac{1+a_2\log(x)}{4\pi\,r^3}\right]\quad\rmn{and}\nonumber\\
x &=& r/\rvir\,.
\end{eqnarray} 
...
\begin{figure}
  ...
  \caption{\it Luminosity dominated radius for different mass
    scales. We show the normalized luminosity per logaritmic radial
    bin which corresponds to $x^3\rho^2/\rho^2_{200}$ where
    $\rho_{200}$ is the density at $\rvir$. The solid lines represents
    cuspy $\beta=1.0$ NFW density profiles for three different mass
    ...
    $M_\rmn{lim}=10^{-6}\,\msun$. The peaks show that the flux is not
    dominated by the center of the halos, but instead by the core
    region for the smooth density profile and by the outer parts for
    the substructures.}
...
accurately measured light from galactic dust and starlight in the ISM
to model the ICM. In Fig.~\ref{fig:SD_spectra} we characterize the
spectral shape through a fit to the galactic spectra presented in
\cite{2006ApJ...648L..29P}. We fix the normalization for the energy
density of light from stars and dust in a galaxy cluster individually
using observed luminosities from SD. In addition, we attach a mass
scaling to the normalization which we adopt from a simple
mass-to-starlight luminosity scaling relation derived from
observations of the brightest cluster galaxy
\cite{2010ApJ...713.1037H}. The figure is showing the spectra at
$r=0.03\rvir$, which is the radius there the SD energy density equals
the energy density of the CMB black-body distribution in a galaxy
cluster. Inside this central radius the SD component is dominating,
which is shown in Fig.~\ref{fig:SD_Edens} where we compare the energy
densities from different radiation fields in a galaxy cluster with the
mass $\mvir=10^{15}\msun$. For this figure we use two different
profiles for the SD energy density, where the total profile includes
the contribution from the ICL, the BCG and all the galaxies, while the
galaxies are excluded in the smooth profile. We find that even for a
cluster with a relative small mass the energy density of the SD
components dominates over the CMB and the magnetic fields (with a
central B field of 3~$\mu G$) within about 10\% of $\rvir$. Outside
this radius the CMB is dominating the energy density of the cluster,
hence the magnetic fields are always subdominant in the cluster and
can safely be removed from cluster IC calculations for a reasonable
central B-field and spectral index. Note that in this work we keep the
contribution from B-fields to the total energy density for
consistency. Also note that we extract the spatial distribution of the
SD light in clusters from a stacked emission analysis of Sloan Digital
Sky Survey (SDSS) data at the redshift $\sim 0.25$
\cite{2005MNRAS.358..949Z} (c.f. Fig.~\ref{fig:SD_spatial}).

ADD DISCUSSION ABOUT ACCUMILATIVE LUMINOSITY
...
\caption{\it Spectral dependence of radiation fields in a cluster of
  ...
  through Eqns.~(\ref{eq:u_SD}-\ref{eq:N_stars}), where we use have
  used the mass $\mvir=10^{15}\msun$ in this figure. Finally we
...
\caption{\it Accumulative luminosity as a function of radius. We show
  ...
  line show the emission from stars and dust (SD) and the IC
  upscattered SD light is shown by the dashed lines. The red lines
  show the total emission including the brightest cluster galaxy, the
  intra cluster light and the additional intra cluster galaxies, while
  these galaxies have been cut out in the smooth component shown by
  the orange lines. We normalize the SD light and the IC upscattered
  SD light with the luminosity from the smooth component within
  $\rvir$ for each component. We find that the gamma-ray IC
  luminosities are dominated by the central regime, while the SD light
  is mainly build up in the outer parts of the cluster. In addition we
  show that the SD light trace the cuspy NFW density profile (blue)
  outside 10\% of $\rvir$.}
 \label{fig:SD_lum}
\end{figure}
...
order to maximize potential DM signals over expected background.
...
\caption{\it Comparing the differential flux from different models: we
  ...
  substructures of 570 (ADD SUBBOOST ON SD MODEL, SHOULD BE ONLY FACTOR
  FEW). We find bright prospects for detecting the continuum emission
  that is dominating the total emission in the GeV energy range. Also
  note that, above 100~MeV the total inverse Compton emission is at
  least a factor 10 smaller than the emission expected from both the
  continuum emission and the emission coming from CRps.}
 \label{fig:diff_BM}
\end{minipage}
\end{figure*}
...
\caption{\it Comparing the emission from different clusters. We show
  ...
  regime is most constraining, and impose upper limits on boost
  factors.}
 \label{fig:clu_comp}
\end{minipage}
\end{figure*}
...
\caption{\it Constraining boost factors using flux upper limits. The
  ...
  smallest size of halos to be larger than $100\,\msun$. However, if
  the smallest size of DM halos is $10^{-6}\,\msun$, we can constrain
  the SFE to about 2 in Fornax, corresponding to a maximum SFE in the
  MW of about 5.}
...
\begin{minipage}{2.0\columnwidth}
 ...
 \caption{\it Comparison of the gamma-ray flux induced by CRps for the
   106 clusters included in the extended HIFLUGCS catalogue. The
   fluxes are calculated within $\rvir$, and are derived using a
   single beta profile for each cluster's gas density profile as
   obtained by ROSAT X-ray observations. The CR distribution follows
   the semi-analytically model derived by hydrodynamical cosmological
   cluster simulations \cite{2010MNRAS.409..449P} (see text for
   details). The upper panel shows the energy integrated flux above
   100~MeV (left side) and above 100~GeV (right side), both as a
   function of HIFLUGCS cluster ID. The eight brightest clusters are
   labelled. The lower panel shows the relative difference between the
   $\gamma$-ray flux predicted by the mass-luminosity scaling relation
   and to the flux computed using the semi-analytical model
   \cite{2010MNRAS.409..449P}. The name of the clusters with the
   largest positive offset are printed out; as expected, they all
