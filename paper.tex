...
\begin{abstract}
  ...
  sensitivity drops linearly with radius and they typically have
  $5-10$ linear resolution elements across a cluster; the gamma-ray
  satellite Fermi is better suited. Assuming cold dark matter with a
  substructure mass distribution down to an Earth mass and using Fermi
  upper limits, we rule out the leptophilic models in their present
  form in 14 clusters, and limit the boost from Sommerfeld enhancement
  in Fornax to be less than 2 which corresponds to a value in the
  Milky Way of less than 5. Alternatively, if the Sommerfeld
  enhancement factor is realized in Nature, this would imply a
  limiting substructure mass in Fornax of $\mlim > 10^2\,\msun$ which
  would be a challenge for structure formation. We find that Fermi
  three-year data will be able to constrain the smallest mass of
  substructures for supersymmetric benchmark models or alternatively
  constrain their cross sections for a given substructure boost. The
  Fermi upper limits are closing in on our predictions for the CR flux
  using an analytic model based on cosmological hydrodynamical cluster
  simulations. We will soon start to constrain the underlying CR
...
built up within $0.1\rvir$ (i.e. close to $r_\s$). Hence the slope of
...
periphery of the cluster, where the difference is mainly built up at a
...
\caption{\it Comparing the flux from different inverse Compton
  ...
  from Fermi \protect \cite{2010ApJ...717L..71A} indicating that the model
...
the entire sample.  Assuming the projected point
...
{\bf Christoph, are you sure about this? CTA will be able to collect data in survey mode with a large angular acceptance (6-8 degrees). From the Concept paper that they put on the arXiv last fall:
``CTA will also have the potential to deliver for the first time significant results on the
extragalactic diffuse emission at VHE, with the possibility to probe the integrated emission
from all sources at these energies. While well measured at GeV energies with the EGRET
and Fermi instruments, the diffuse emission at VHE is extremely challenging to measure
due to its faintness and the difficulty to adequately subtract the background. Here, the
improved sensitivity coupled with the large field of view puts detection in reach of CTA.'' So, maybe we should instead say that we want encourage the development of new methods to detect very extended sources?}
...
the point spread function $\sim 0.1\deg$, unless new background subtraction schemes can be found, e.g., for the survey mode of CTA. {\bf Note new last part of this senstence!}
...
\begin{figure}%[t]
 ...
 \caption{\it Comparing different electron number density profiles for
   ...
   observations \protect \cite{2002ApJ...565..883P}. The total hybrid
   profile shown by the red solid line represent the best fit to the
   data, where the fitted individual density components are shown by
   the black dotted lines. The blue solid line shows the single beta
