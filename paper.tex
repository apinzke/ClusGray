...
\caption{\it Comparing the gamma-ray emission from different
  clusters. We show the differential flux of clusters: Fornax (upper
  left), Coma (upper right), Virgo (lower left), and Perseus (lower
  right). We show CR induced emission (red solid), a leptophilic (LP)
  model that includes both final state radiation and IC upscattered
  CMB, dust and starlight (green dash-dotted), and the benchmark $\Kp$
  model (BM) that includes continuum emission, and IC upscattered CMB,
  dust and starlight (blue dashed). The arrows show the spatially
  extended differential upper limits set by Fermi-LAT in the energy
  ranges $0.2-1$~GeV, $1-10$~GeV, and $10-100$~GeV from left to
  right. We show the individual components for the BM model and LP
  model for Virgo and Perseus, respectively. The flux from the
...
\C{A quantity that is of great scientific interest is the CR pressure
  ($P_\CR$) relative to the thermal pressure ($P_\rmn{th}$), $X_\CR =
  P_\CR/P_\rmn{th}$, as it directly bias galaxy cluster observables
  and the resulting cosmological constraints, such as the hydrostatic
  mass bias or the Sunyaev-Zel’dovich signal measured by Planck, South
  Pole Telescope, or Atacama Cosmology Telescope. In the following we
  estimate the mass scaling of the relative CR pressure using the CR
  ...
  10^{14}\,\msun$. We determine the mass scaling of the gas density
  through its relation to the enclosed gas fraction $f_\rmn{gas}$, and
  find that $\rho_\rmn{gas}\propto f_\rmn{gas} \propto M^{0.135}$
  \cite{2009ApJ...693.1142S}. The thermal pressure is derived from the
  temperature in virial equilibrium, and scales as $P_\rmn{th}\propto
  k_\B T \propto G M/R \propto M^{0.67}$. Finally we can derive the
  mass scaling of the volume weighted relative CR pressure from
\begin{equation}
...
simulated cluster \cite{2008MNRAS.385.1211P}.  Since the CRs that
...
are derived. We find that the most constraining clusters are the Norma
and Coma clusters of $<X_\CR> < 0.07$.}
...
clusters/groups but Fornax. \C{We conclude that due to the large
  ...
  without improved modelling. However, we can put tight constrints on
